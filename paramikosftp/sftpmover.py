...
def transfer_to_this_remote_dir():
    the_directory = input("Enther the full path of the destination: ")
    if the_directory[0] == "/" and the_directory[-1] == "/":
...
def main():
    ...
    dir_to_use = transfer_to_this_remote_dir()

    ## iterate across the files within directory
    for x in os.listdir("/home/student/filestocopy/"): # iterate on directory contents
      if not os.path.isdir("/home/student/filestocopy/"+x): # filter everything that is NOT a directory
        sftp.put("/home/student/filestocopy/"+x, dir_to_use + x) # sftp.put("from_path_local", "to_path_remote") - move file to target location
